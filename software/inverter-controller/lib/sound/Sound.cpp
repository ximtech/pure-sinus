...
void lowBatterySound(uint8_t buzzerPin) {
...
void currentTooHighSound(uint8_t buzzerPin) {
...
void overTemperatureSound(uint8_t buzzerPin) {
    tone(buzzerPin, NOTE_C5, HIGH_CURRENT_TONE_DURATION_MS);
    delay(HIGH_CURRENT_TONE_DURATION_MS + HIGH_CURRENT_PAUSE_DURATION_MS);
    tone(buzzerPin, NOTE_C6, HIGH_CURRENT_TONE_DURATION_MS);
    delay(HIGH_CURRENT_TONE_DURATION_MS + HIGH_CURRENT_PAUSE_DURATION_MS);
}

void fatalErrorSound(uint8_t buzzerPin) {
    for (int i = 0; i < 3; i++) {
        tone(buzzerPin, NOTE_C4, 500);
        delay(600);
        tone(buzzerPin, NOTE_C3, 500);
        delay(600);
    }
}
